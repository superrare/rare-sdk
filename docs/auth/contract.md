...
`apiBaseUrl` is the only SDK/CLI service URL. Auth routes and the issuer are derived as `<apiBaseUrl>/auth/v2`. Production defaults to `https://api.superrare.com`; the development API base is `https://rare-api-devmainnet-784573620320.us-east1.run.app`. Production host/routing/key provisioning is deployment work. HTTPS required except loopback local development; no credential-bearing redirects. Static public clients `rare-cli` and `rare-sdk`; no dynamic registration. Requested scope `rare:account offline_access`. Fixed configured resource is Rare API audience. Sessions/SDK persisted envelopes are bound to authority, client and API base.

## Routing boundary
Rare API forwards only POST `/auth/v2/wallet/challenge`, `/auth/v2/device/authorization`, `/auth/v2/token` and `/auth/v2/revoke` to its configured `AUTH_SERVICE` internal base. Auth still owns proof verification and session lifecycle. The proxy preserves OAuth responses, refuses redirects, bounds request/response sizes and time, and does not forward caller cookies, Authorization or service credentials. It never exposes introspection, hosted bridge or legacy routes through a wildcard. Introspection and Connect's protected bridge use `AUTH_SERVICE` directly. No load balancer changes are required for either environment.

The server-side public API base is `RARE_API_PUBLIC_URL`; it supplies the canonical issuer for auth signing and API validation. `RARE_API_URL` remains the provisioning transport URL and may be internal. `AUTH_PUBLIC_URL` is removed; request Host/forwarded headers never select issuer identity. Changing the canonical API base changes the issuer and requires fresh sessions.

## Public authority API
The paths below are relative to the public API's `/auth/v2` prefix.
...
- Internal POST `/introspect` form-urlencoded `{token:accessToken}` directly on `AUTH_SERVICE` (not exposed by the API passthrough). Returns `{active:false}` or `{active:true,sub:decimalAccountId,client_id,scope,iss,aud,exp,address,chain_id,sid}`; chain_id is a positive integer number. JSON response no-store. Active verifies both token and current family/address binding. Introspection resolves the opaque token and its current session family in Redis; raw bearer tokens are not persisted.
...
Auth -> Rare API POST `/internal/v1/accounts/resolve`, shared server-only `AUTH_INTERNAL_API_KEY` Bearer credential (not browser token), JSON `{address,chainId}` -> `{accountId,address,username,created}`. IDs positive decimal strings, lowercase address. Atomic DB creation of account + address, unique-conflict reread, no reassignment, compromised address rejected. Email not required. Existing account data preserved. No legacy token-to-new-token exchange.
...
The Connect server uses a shared server-only `AUTH_INTERNAL_API_KEY` Bearer credential for issuer-relative endpoints:
...
Auth deployment derives `/auth/v2`, `/device` and the account provisioning path from service base URLs. Its audience is fixed to `rare-api`; service secrets have matching names across repositories. SDK/CLI derive auth routes from their single API base; internal auth routing remains server-side configuration.
